...
class FilterElementStatisticsModel extends Model
{
    public function SubmitData($condition)
    {
        $m = M('filter_element_statistics');
        if ($condition['data']) {
            $result = true;
    ...
    {
        $sql = "
        SELECT COUNT(DISTINCT date) as totalrecords  FROM filter_element_statistics";
        if ($condition) {
            $hasWhere = false;
    ...
    {
        $sql = "
        SELECT DISTINCT date  FROM filter_element_statistics";
        if ($condition) {
            $hasWhere = false;
    ...
    public function delete($condition)
    {
        ...
        if ($condition['data']) {
            ...
            for ($i=0; $i <  count($condition['data']); $i++) {
                ...
                $result = M('filter_element_statistics')->where($data)->delete();
                if (!$result) {
                    return $result;
                }
            }
    ...
    public function getStatistics($condition)
    {
        ...
        $sql = "
                ...
                ((SELECT *, SUM(`number`) AS task_number 
                  FROM filter_element_statistics $whereSql GROUP BY `$groupby`
                ) AS tcs) 
              ";
        if (isset($condition['start_record']) && $condition['page_size']) {
            $start = $condition['start_record'];
