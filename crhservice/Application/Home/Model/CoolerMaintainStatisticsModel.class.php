...
class CoolerMaintainStatisticsModel extends Model
{
    ...
    {
        $sql = "
        SELECT COUNT(DISTINCT date) as totalrecords  FROM cooler_maintain_statistics";
        if ($condition) {
            $hasWhere = false;
...
//                    $date = date( "Y-m-d", strtotime( $endStr .  "+1 day" ) );
    ...
    {
        $sql = "
        SELECT DISTINCT date  FROM cooler_maintain_statistics";
        if ($condition) {
            $hasWhere = false;
    ...
    public function delete($condition)
    {
        ...
        if ($condition['data']) {
            ...
            for ($i=0; $i <  count($condition['data']); $i++) {
                ...
                $result = M('cooler_maintain_statistics')->where($data)->delete();
                if (!$result) {
                    return $result;
                }
            }
    ...
    public function getStatistics($condition)
    {
        ...
        $sql = "
                ...
                ((SELECT *, SUM(`number`) AS task_number 
                  FROM cooler_maintain_statistics $whereSql GROUP BY `$groupby`
                ) AS tcs) 
              ";
