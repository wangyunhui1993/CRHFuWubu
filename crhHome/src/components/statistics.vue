        ...
        <el-menu :default-active="$route.path" theme="dark" @select="handleSelect">
          <template v-for="root in $router.options.routes" v-if="!root.hidden">
            <template v-for="sub in root.children" v-if="!sub.hidden">
              <el-menu-item v-for="item in sub.children" :index="item.path" v-if="sub.path == '/home/statistics' && isAuthed(item.path)">{{item.meta}}</el-menu-item>
            </template>
          </template>
        </el-menu>
      </el-col>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      isAuthed(path) {
          //Task
          let auths = JSON.parse(getAuthArray(1));
          let found = false;
    ...
    created: function () {
      if (window.location.hash != null) {
        ...
        for(let i=0; i < _this.$router.options.routes.length; i++) {
          if(!_this.$router.options.routes[i].hidden && _this.$router.options.routes[i].children.length > 0) {
            for(let j=0; j< _this.$router.options.routes[i].children.length; j++) {
              if(_this.$router.options.routes[i].children[j].path == "/home/statistics") {
        ...
        if(!hit) {
          //TODO:跳转至第一个有权限的URL
            let firstPath = JSON.parse(getAuthArray(1))[0];
//          alert(firstPath)
